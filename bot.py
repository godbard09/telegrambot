...
async def current_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        # Phân tích tín hiệu gần nhất
        position_info = "Không có tín hiệu mua/bán trong 7 ngày qua."
        if signals:
            latest_signal = signals[-1]
            if latest_signal['type'] == "MUA":
                profit_loss = ((current_price - latest_signal['price']) / latest_signal['price']) * 100
                profit_color = (
                ...
                position_info = (
                    ...
                    f"- Ngày mua: {latest_signal['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}\n"
                    f"- Giá mua: {latest_signal['price']:.2f} {quote_currency}\n"
                    f"- Lãi/Lỗ: {profit_color}"
                )
            elif latest_signal['type'] == "BÁN":
                # Tìm tín hiệu MUA trước đó
                previous_buy = next((s for s in reversed(signals[:-1]) if s['type'] == "MUA"), None)
                if previous_buy:
                    profit_loss = ((latest_signal['price'] - previous_buy['price']) / previous_buy['price']) * 100
                    profit_color = (
                        f"{profit_loss:.2f}% 🟢" if profit_loss > 0 else
                        f"{profit_loss:.2f}% 🔴" if profit_loss < 0 else
                        f"{profit_loss:.2f}% 🟡"
                    ...
                    position_info = (
                        ...
                        f"- Ngày mua: {previous_buy['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}\n"
                        f"- Giá mua: {previous_buy['price']:.2f} {quote_currency}\n"
                        f"- Ngày bán: {latest_signal['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}\n"
                        f"- Giá bán: {latest_signal['price']:.2f} {quote_currency}\n"
                        f"- Lãi/Lỗ: {profit_color}"
                    )
                else:
                    position_info = (
                        ...
                        f"- Ngày bán: {latest_signal['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}\n"
                        f"- Giá bán: {latest_signal['price']:.2f} {quote_currency}\n"
                        f"- Không tìm thấy tín hiệu MUA trước đó."
                    )
